...
def model_prepare():
  ...
  input_layer = keras.Input(shape = (351,2,1), name='main_input')
  x     = layers.Conv2D(16, 2, padding='same', activation='relu')(input_layer)
  x     = layers.Dropout(0.33)(x)
  x     = layers.Conv2D(32, 2, padding='same', activation='relu')(x)
  x     = layers.Conv2D(8, 2, padding='same', activation='relu')(x)
...
def train_net(model, files):
  ...
  history = model.fit(X_train, y_train, validation_data=(X_valid, y_valid), batch_size=30, epochs=150, verbose=1)
  end = time.time()
